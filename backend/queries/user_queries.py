...
from models.user_model import User, UpdateUserProcesses
...
async def get_all_users():
    ...
    async for document in collection.find({}):
        # Crear una instancia de User con los datos del documento
        user = User(**document)
        users.append(user)
    return users
...
async def update_user_processes(email: str, data: UpdateUserProcesses):

    processes = data.processes

    await collection.update_one({'email': email}, {'$addToSet': {'processes': {'$each': processes}}})
